<x-admin-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm"><p class="text-gray-500 text-sm">Kehadiran Hari 1</p><p id="hadir-hari-1" class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[1] ?? 0 }}</p></div>
        <div class="bg-white p-6 rounded-lg shadow-sm"><p class="text-gray-500 text-sm">Kehadiran Hari 2</p><p id="hadir-hari-2" class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[2] ?? 0 }}</p></div>
        <div class="bg-white p-6 rounded-lg shadow-sm"><p class="text-gray-500 text-sm">Kehadiran Hari 3</p><p id="hadir-hari-3" class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[3] ?? 0 }}</p></div>
    </div>

    <div class="bg-white p-6 rounded-lg shadow-sm">
        <div class="border-b border-gray-200 mb-6">
            <nav class="-mb-px flex space-x-6" id="tabs">
                <a href="#absen-manual" class="tab-link py-2 px-1 border-b-2 border-teal-500 font-semibold text-teal-600">Absen Manual</a>
                <a href="#absen-massal" class="tab-link py-2 px-1 border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">Absen Massal</a>
                <a href="#data-kehadiran" class="tab-link py-2 px-1 border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300">Data Kehadiran</a>
        ...
        <div id="tab-contents">
            <div id="absen-manual" class="tab-content">
                ...
                <p class="text-gray-500 mb-4">Ketik token peserta untuk melakukan check-in. Hasil akan muncul otomatis.</p>
                
                <div class="p-4 border rounded-lg">
                    <div class="flex items-end space-x-3">
                        <div class="flex-grow">
                            <label for="token-search-input" class="text-sm font-medium">Token Peserta</label>
                            <input type="text" id="token-search-input" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500" placeholder="Mulai ketik token...">
                        </div>
                        <div>
                            <label for="day-select" class="text-sm font-medium">Hari Event</label>
                            <select id="day-select" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500">
                    ...
                    <div id="search-result-container" class="mt-4 pt-4 border-t border-gray-200 min-h-[80px] flex items-center justify-center">
                    ...
                    <div class="p-4 border rounded-lg">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                <input type="search" id="mass-search-input" placeholder="Nama, token, HP, kabupaten..." class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500">
                            </div>
                            <div>
                                <label class="text-sm font-medium">Hari Event</label>
                                <select name="day" id="mass-day-select" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-sky-500 focus:border-sky-500">
                                    <option value="1">Hari 1</option>
                                    <option value="2">Hari 2</option>
                                    <option value="3">Hari 3</option>
                                </select>
                            </div>
                            <button type="submit" id="mass-submit-btn" class="self-end px-6 py-2 bg-gray-400 text-white rounded-lg font-semibold h-10.5 disabled:opacity-50" disabled>
                                <i class="fa-solid fa-save mr-2"></i>Simpan (0)
                            </button>
                        </div>
                        <div id="mass-attendees-container" class="border-t pt-4">
                            <div class="mb-2"><input type="checkbox" id="check-all-mass" class="rounded"><label for="check-all-mass" class="ml-2 font-medium">Pilih Semua</label></div>
             ...
             <div id="data-kehadiran" class="tab-content hidden">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <form method="GET" action="{{ url()->current() }}" id="filter-log-form">
                        ...
                        <select name="filter_log_day" onchange="document.getElementById('filter-log-form').submit()" class="border-gray-300 rounded-lg shadow-sm">
                            <option value="all" {{ request('filter_log_day', 'all') == 'all' ? 'selected' : '' }}>Semua Hari</option>
                            <option value="1" {{ request('filter_log_day') == 1 ? 'selected' : '' }}>Hari 1</option>
                            <option value="2" {{ request('filter_log_day') == 2 ? 'selected' : '' }}>Hari 2</option>
                            <option value="3" {{ request('filter_log_day') == 3 ? 'selected' : '' }}>Hari 3</option>
                        </select>
                    </form>
                </div>
                <div class="space-y-3">
                    ...
                    <div class="p-3 border rounded flex flex-wrap justify-between items-center gap-4">
                        <div class="flex items-center"><div class="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center"><i class="fa-solid fa-check"></i></div><div class="ml-3"><p class="font-semibold">{{ $log->attendee->name }}</p><p class="text-xs text-gray-500">{{ $log->attendee->token }} &bull; {{ $log->attendee->phone_number }}</p></div></div>
                        <div><span class="text-xs font-semibold bg-gray-800 text-white px-2 py-1 rounded-full mr-4">Hari {{ $log->day }}</span><span class="text-sm text-gray-500">{{ $log->created_at->format('d/m/Y, H:i') }}</span></div>
                    </div>
                    @empty
                    <p class="text-center text-gray-400 py-8">Belum ada data kehadiran yang cocok dengan filter.</p>
                    @endforelse
                </div>
            ...
            <div id="analisis" class="tab-content hidden">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            ...
                            <span class="text-sm text-gray-600">{{ $dailyAttendance[$i] ?? 0 }} dari {{ $totalAttendees }} ({{ $totalAttendees > 0 ? number_format((($dailyAttendance[$i] ?? 0) / $totalAttendees) * 100, 0) : 0 }}%)</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-4"><div class="bg-teal-500 h-4 rounded-full" style="width: {{ $totalAttendees > 0 ? (($dailyAttendance[$i] ?? 0) / $totalAttendees) * 100 : 0 }}%"></div></div>
                        <div class="flex justify-between text-xs text-gray-500 mt-1">
                            <span>Hadir: {{ $dailyAttendance[$i] ?? 0 }}</span>
                            <span>Tidak Hadir: {{ $totalAttendees - ($dailyAttendance[$i] ?? 0) }}</span>
                        </div>
                        
                        <a href="{{ route('admin.export.attendance.analysis', ['day' => $i]) }}" class="text-xs text-blue-600 hover:underline mt-2 inline-block">
                            <i class="fa-solid fa-download mr-1"></i>Export Analisis Wilayah Hari {{ $i }}
                        </a>
                        </div>
                    @endfor
                </div>
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <form method="GET" action="{{ url()->current() }}" id="analysis-form">
                        ...
                        <select name="analysis_day" onchange="document.getElementById('analysis-form').submit()" class="border-gray-300 rounded-lg shadow-sm">
            ...
            <div id="laporan" class="tab-content hidden">
                ...
                <div class="border-t pt-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <a href="{{ route('admin.export.attendance.by_day', ['day' => 1]) }}" class="block p-6 bg-teal-50 border border-teal-200 rounded-lg hover:bg-teal-100 text-teal-800 transition text-center">
                        ...
                        <a href="{{ route('admin.export.attendance.by_day', ['day' => 2]) }}" class="block p-6 bg-teal-50 border border-teal-200 rounded-lg hover:bg-teal-100 text-teal-800 transition text-center">
                            <i class="fa-solid fa-file-excel fa-2x mb-3"></i>
                            <p class="font-semibold">Laporan Hari 2</p>
                            <p class="text-sm opacity-80">Download data kehadiran</p>
                        </a>
                        <a href="{{ route('admin.export.attendance.by_day', ['day' => 3]) }}" class="block p-6 bg-teal-50 border border-teal-200 rounded-lg hover:bg-teal-100 text-teal-800 transition text-center">
                            <i class="fa-solid fa-file-excel fa-2x mb-3"></i>
                ...
                <div class="">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        <a href="{{ route('admin.export.users.complete') }}" class="block p-6 bg-gray-50 border rounded-lg hover:bg-gray-100 text-center transition">
                        ...
                        <a href="{{ route('admin.export.users.demographics') }}" class="block p-6 bg-gray-50 border rounded-lg hover:bg-gray-100 text-center transition">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const fetchMassAttendees = async () => {
            ...
            massAttendeesList.innerHTML = `<p class="text-center text-gray-500 py-4"><i class="fa-solid fa-spinner fa-spin mr-2"></i>Memuat peserta...</p>`;

            try {
                const response = await fetch(`{{ route('admin.attendance.findForMassAbsen') }}?day=${day}&search=${search}`);
                const attendees = await response.json();
                renderMassAttendees(attendees);
            } catch (error) {
                massAttendeesList.innerHTML = `<p class="text-center text-red-500 py-4">Gagal memuat data.</p>`;
            }
        };
        ...
        function renderMassAttendees(attendees) {
            ...
            if (attendees.length === 0) {
                massAttendeesList.innerHTML = `<p class="text-center text-gray-400 py-4">Tidak ada peserta yang bisa dipilih.</p>`;
                return;
            }
        ...
        const searchAttendee = async () => {
            ...
            if (token.length < 3) {
                resultContainer.innerHTML = '<p class="text-sm text-gray-500">Ketik minimal 3 karakter token untuk memulai pencarian.</p>';
                return;
            }
            ...
            try {
                const response = await fetch(`{{ route('admin.attendance.findByToken') }}?token=${token}&day=${day}`);
                ...
                resultContainer.innerHTML = '<p class="text-sm text-red-500">Terjadi kesalahan saat mencari.</p>';
            }
        };
        ...
        window.markAsPresent = async (buttonElement, token, day) => {
            ...
            try {
                const response = await fetch(`{{ route('admin.attendance.storeAjax') }}`, {
                    ...
                    body: JSON.stringify({ token: token, day: day })
                });
        ...
        function updateStatCards(day) {
            ...
            if(card) {
                ...
                card.closest('.bg-white').classList.add('transform', 'scale-105', 'bg-green-50', 'transition-all', 'duration-300');
                setTimeout(() => {
    ...
    new Chart(ageCtx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Peserta',
                data: ageData.data,
                backgroundColor: ['#60A5FA', '#A78BFA', '#FBBF24', '#34D399', '#F87171', '#93C5FD'],
                hoverOffset: 4
            }]
        },
        options: {
