<x-admin-layout>
    ...
    <div class="mb-6 flex border-b">
        <a href="{{ route('admin.dashboard', ['day' => 'all']) }}"
            class="px-4 py-2 font-semibold border-b-2 {{ $selectedDay == 'all' ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Semua
            Hari</a>
        <a href="{{ route('admin.dashboard', ['day' => 1]) }}"
            class="px-4 py-2 font-semibold border-b-2 {{ $selectedDay == 1 ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Hari
            1</a>
        <a href="{{ route('admin.dashboard', ['day' => 2]) }}"
            class="px-4 py-2 font-semibold border-b-2 {{ $selectedDay == 2 ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Hari
            2</a>
        <a href="{{ route('admin.dashboard', ['day' => 3]) }}"
            class="px-4 py-2 font-semibold border-b-2 {{ $selectedDay == 3 ? 'border-sky-500 text-sky-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">Hari
            3</a>
    </div>

    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Total Kehadiran @if ($selectedDay !== 'all')
                    (Hari Ke-{{ $selectedDay }})
                @endif
            </p>
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Total Penjualan @if ($selectedDay !== 'all')
                    (Hari Ke-{{ $selectedDay }})
                @endif
            </p>
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Tingkat Kehadiran @if ($selectedDay !== 'all')
                    (Hari Ke-{{ $selectedDay }})
                @endif
            </p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div class="bg-white p-6 rounded-lg shadow-sm">
                ...
                @foreach ($dailyAttendance as $dayLabel => $count)
                    <li class="flex justify-between items-center">
                        <div>
                            ...
                            @if ($dayLabel == 'Hari ' . $eventDayNow)
                                <span
                                    class="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full ml-2">Berlangsung</span>
                            @endif
                        </div>
            ...
            <a href="{{ route('admin.attendance.index') }}"
                class="text-teal-600 font-semibold text-sm mt-4 inline-block">Kelola Presensi <i
                ...
                @foreach ($dailySales as $dayLabel => $sale)
                    <li class="flex justify-between items-center">
                        <div>
                            <span class="font-semibold">{{ $dayLabel }}</span>
                            @if ($dayLabel == 'Hari ' . $eventDayNow)
                                <p class="text-xs text-blue-500">Berlangsung</p>
                            @endif
                        </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm mb-6">
        <h3 class="font-bold text-lg mb-4">Tenan Terlaris @if ($selectedDay !== 'all')
                (Hari Ke-{{ $selectedDay }})
            @else
                (Semua Hari)
            @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @foreach ($topTenants as $category => $tenants)
                <div>
                    <h4 class="font-semibold capitalize mb-3 border-b pb-2">
                        {{ Str::title(str_replace('_', ' ', $category)) }}</h4>
                    <ul class="space-y-3">
                            ...
                            @if ($tenant->total_sales > 0)
                                <li class="flex justify-between items-center text-sm">
                                    <span class="truncate pr-2">
                                        <span
                                            class="bg-gray-200 rounded-full w-5 h-5 inline-flex items-center justify-center mr-2 font-bold">{{ $index + 1 }}</span>
                                        {{ $tenant->tenant_name }}
                                    </span>
                                    <span class="font-semibold text-teal-600 whitespace-nowrap">Rp
                                        {{ number_format($tenant->total_sales, 0, ',', '.') }}</span>
                                </li>
                            @endif
                        @empty
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <h3 class="font-bold text-lg mb-4">
                Event Hari Ini
                @if ($eventDayNow > 0)
                    <span class="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full ml-2">Hari
                        Ke-{{ $eventDayNow }} Sedang Berlangsung</span>
                @endif
            </h3>
            <div class="space-y-4">
                @forelse($todaysEvents as $event)
                    <div class="border-b pb-3 last:border-b-0">
                        <div class="flex justify-between items-start">
                            ...
                            <span class="text-xs font-semibold bg-gray-100 px-2 py-1 rounded-full">Hari
                                {{ $event->day }}</span>
                        </div>
            ...
            <a href="{{ route('admin.events.index') }}"
                class="text-teal-600 font-semibold text-sm mt-4 inline-block">Kelola Semua Event <i
                    class="fa-solid fa-arrow-right ml-1"></i></a>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div class="grid grid-cols-2 gap-4">
                <a href="{{ route('admin.users.index') }}"
                    class="text-center p-4 bg-gray-50 rounded-lg hover:bg-gray-100">
