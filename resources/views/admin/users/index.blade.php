<x-admin-layout>
        ...
        <div class="flex space-x-2">
            <a href="{{ route('admin.users.create') }}"
                class="px-4 py-2 bg-gray-800 text-white rounded-lg font-semibold hover:bg-gray-700"><i
                    class="fa-solid fa-plus mr-2"></i>Tambah Peserta</a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Hadir Hari 1</p>
            <p class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[1] ?? 0 }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Hadir Hari 2</p>
            <p class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[2] ?? 0 }}</p>
        </div>
        <div class="bg-white p-6 rounded-lg shadow-sm">
            <p class="text-gray-500 text-sm">Hadir Hari 3</p>
            <p class="text-3xl font-bold text-gray-800">{{ $dailyAttendance[3] ?? 0 }}</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div class="border-b border-gray-200 mb-6">
            <nav class="-mb-px flex space-x-6" id="tabs">
                <a href="#daftar"
                    class="tab-link py-2 px-1 border-b-2 border-teal-500 font-semibold text-teal-600">Daftar Peserta</a>
                <a href="#analisis"
        ...
        <div id="tab-contents">
            ...
            <div id="daftar" class="tab-content">
                ...
                <form action="{{ route('admin.users.index') }}" method="GET">
                    <div
                        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end p-4 border rounded-lg mb-6">
                        <div><label for="search" class="text-sm font-medium">Pencarian</label><input type="text"
                                name="search" id="search" value="{{ request('search') }}"
                                placeholder="Nama, token, HP" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm">
                        </div>
                        <div><label for="regency" class="text-sm font-medium">Kabupaten</label><select name="regency"
                                id="regency" class="mt-1 w-full border-gray-300 rounded-lg shadow-sm">
                                <option value="Semua">Semua Kabupaten</option>
                                @foreach ($regencies as $regency)
                                    <option value="{{ $regency }}"
                                        {{ request('regency') == $regency ? 'selected' : '' }}>{{ $regency }}
                                    </option>
                                @endforeach
                            </select></div>
                        <div><label for="attendance_status" class="text-sm font-medium">Kehadiran</label><select
                                ...
                                <option value="Hadir" {{ request('attendance_status') == 'Hadir' ? 'selected' : '' }}>
                                    Sudah Hadir</option>
                                <option value="Tidak Hadir"
                                    {{ request('attendance_status') == 'Tidak Hadir' ? 'selected' : '' }}>Belum Hadir
                                </option>
                            </select></div>
                        <button type="submit"
                    ...
                    @forelse($attendees as $attendee)
                        ...
                        <div
                            ...
                            <div class="flex items-center space-x-4">
                                <div
                                    class="w-12 h-12 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center font-bold text-lg flex-shrink-0">
                                    {{ substr($attendee->name, 0, 1) }}{{ str_contains($attendee->name, ' ') ? substr(explode(' ', $attendee->name)[1], 0, 1) : '' }}
                                </div>
                                <div>
                                    <p class="font-bold text-gray-800">{{ $attendee->name }} <span
                                            class="text-xs font-mono bg-gray-100 px-2 py-1 rounded">{{ $attendee->token }}</span>
                                    </p>
                                    <div class="flex flex-wrap items-center gap-x-4 text-sm text-gray-500 mt-1">
                                        <span><i
                                                class="fa-solid fa-location-dot mr-1"></i>{{ $attendee->regency }}</span>
                                        <span><i
                                                class="fa-solid fa-phone mr-1"></i>{{ $attendee->phone_number }}</span>
                                        <span><i class="fa-solid fa-calendar-day mr-1"></i>{{ $attendee->age }}
                                            tahun</span>
                                    </div>
                                    <div class="flex items-center space-x-3 text-sm mt-2">
                                        <span class="font-semibold">Kehadiran:</span>
                                        <span class="{{ $hadir1 ? 'text-green-600' : 'text-red-500' }}"><i
                                                class="fa-solid {{ $hadir1 ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>H1</span>
                                        <span class="{{ $hadir2 ? 'text-green-600' : 'text-red-500' }}"><i
                                                class="fa-solid {{ $hadir2 ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>H2</span>
                                        <span class="{{ $hadir3 ? 'text-green-600' : 'text-red-500' }}"><i
                                                class="fa-solid {{ $hadir3 ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>H3</span>
                                        <span
                                            class="text-xs font-bold bg-gray-800 text-white px-2 py-1 rounded-full">{{ $totalHadir }}/3
                                            hari</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex space-x-2 flex-shrink-0">
                                <a href="{{ route('admin.users.edit', $attendee->id) }}"
                                    class="px-4 py-2 border border-gray-300 rounded-lg font-semibold text-gray-700 text-sm hover:bg-gray-100"><i
                                        class="fa-solid fa-pencil mr-2"></i>Edit</a>
                                <form action="{{ route('admin.users.destroy', $attendee->id) }}" method="POST"
                                    ...
                                    <button type="submit"
                                        class="px-4 py-2 border border-red-300 rounded-lg font-semibold text-red-700 text-sm hover:bg-red-50"><i
                                            class="fa-solid fa-trash mr-2"></i>Hapus</button>
                                </form>
                            </div>
                        </div>
                    @empty
                        <div class="text-center py-12 text-gray-500"><i class="fa-solid fa-user-slash fa-3x mb-4"></i>
                            <p>Tidak ada data peserta yang cocok dengan filter Anda.</p>
                        </div>
                    @endforelse
                </div>
                            ...
                            @foreach ($regencyDistribution as $dist)
                                <div class="flex items-center">
                                    ...
                                    <div class="w-2/3 bg-gray-200 rounded-full h-4">
                                        <div class="bg-teal-500 h-4 rounded-full text-xs text-white text-right pr-2"
                        ...
                        <div class="space-y-3">
                            @foreach ($ageDistribution as $dist)
                                <div class="flex items-center">
                                    ...
                                    <div class="w-2/3 bg-gray-200 rounded-full h-4">
                                        <div class="bg-blue-500 h-4 rounded-full text-xs text-white text-right pr-2"
                                            style="width: {{ $totalAttendees > 0 ? ($dist->total / $totalAttendees) * 100 : 0 }}%">
                                        </div>
                                    </div>
                                    <span class="w-16 text-right font-semibold">{{ $dist->total }}</span>
