    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <header class="flex justify-between items-center py-6 relative z-10">
            ...
            <nav class="hidden md:flex items-center space-x-6 text-gray-600">
                ...
                <a href="{{ route('lottery.check') }}" class="bg-gray-900 text-white px-5 py-3 rounded-xl font-semibold shadow-lg hover:bg-gray-800 transition-all duration-300"><i class="fa-solid fa-ticket mr-2"></i>Cek Undian</a>
            </nav>
        ...
        <div id="mobile-menu" class="hidden md:hidden bg-white rounded-2xl shadow-lg mt-2 absolute top-24 left-4 right-4 z-20">
            <nav class="flex flex-col p-6 space-y-5">
                ...
                <a href="{{ route('lottery.check') }}" class="text-gray-700 bg-sky-50 text-sky-600 font-semibold px-4 py-3 rounded-lg"><i class="fa-solid fa-ticket fa-fw mr-3"></i>Cek Undian</a>
            ...
            <div class="max-w-4xl mx-auto">
                @if(!$isDrawTime)
                    <div class="bg-white p-8 sm:p-10 rounded-2xl shadow-xl text-center">
                        <div class="mx-auto w-24 h-24 bg-gradient-to-br from-sky-100 to-blue-100 rounded-full flex items-center justify-center mb-6">
                        ...
                        <p class="text-gray-600 mt-2 max-w-lg mx-auto">Daftar pemenang akan diumumkan setelah event berakhir. Pastikan Anda telah melakukan presensi untuk kesempatan menang!</p>
                        <div class="mt-8 text-lg text-gray-700 bg-gray-50 p-6 rounded-xl border">
                            <p>Nantikan pengumuman pemenang pada:</p>
                            <p class="font-bold text-2xl text-sky-600 mt-2">{{ $drawTime->format('d F Y, H:i') }} WIB</p>
                        </div>
                    </div>
                @else
                    <div class="bg-white p-8 sm:p-10 rounded-2xl shadow-xl text-center">
                        <div class="mx-auto w-24 h-24 bg-gradient-to-br from-amber-100 to-yellow-100 rounded-full flex items-center justify-center mb-6">
                            <i class="fa-solid fa-trophy text-5xl text-amber-500"></i>
                        </div>
                        <h2 class="text-3xl font-bold text-gray-800">Cek Status Undian Anda</h2>
                        <p class="text-gray-600 mt-2">Masukkan nomor token Anda untuk melihat apakah Anda pemenangnya.</p>
                        <form action="{{ route('lottery.check') }}" method="GET" class="mt-8 max-w-lg mx-auto flex">
                            <input type="text" name="token" value="{{ $searchedToken }}" placeholder="Contoh: MJE-000001" class="flex-grow px-4 py-3 border border-gray-300 rounded-l-lg shadow-sm focus:ring-sky-500 focus:border-sky-500 text-lg">
                            <button type="submit" class="bg-gray-900 text-white font-bold py-3 px-6 rounded-r-lg hover:bg-gray-700 transition-all duration-300">
                    ...
                    @if($searchResult === 'win')
                    <div class="mt-8 bg-green-50 border-l-4 border-green-400 text-green-800 p-6 rounded-lg shadow-lg text-center">
                        <h3 class="text-2xl font-bold">SELAMAT! ANDA MENANG!</h3>
                        <p class="mt-2">Token <strong>{{ $searchedToken }}</strong> terdaftar sebagai pemenang. Silakan hubungi panitia untuk informasi lebih lanjut.</p>
                    </div>
                    @elseif($searchResult === 'lose')
                    <div class="mt-8 bg-amber-50 border-l-4 border-amber-400 text-amber-800 p-6 rounded-lg shadow-lg text-center">
                        <h3 class="text-2xl font-bold">MOHON MAAF, ANDA BELUM BERUNTUNG</h3>
                        <p class="mt-2">Token <strong>{{ $searchedToken }}</strong> tidak terdaftar sebagai pemenang. Terima kasih atas partisipasi Anda!</p>
                    </div>
                    @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            ...
                            <div class="bg-white p-6 rounded-2xl shadow-lg winner-card">
                                <div class="flex items-center space-x-4">
                                    <div class="bg-gradient-to-br from-amber-400 to-yellow-400 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-md">
                                        <i class="fa-solid fa-award fa-2x"></i>
                                    </div>
                                    <div>
                                        <p class="font-bold text-lg text-gray-800">{{ Str::mask($winner->attendee->name, '*', 3, -1) }}</p>
                                        <p class="text-sm text-gray-500">{{ $winner->attendee->regency }}</p>
                                    </div>
                                </div>
                                <div class="mt-4 border-t pt-4 text-sm text-gray-600">
                                    <p class="font-semibold">Memenangkan: <span class="text-sky-600">{{ $winner->prize->name }}</span></p>
                                    <p class="text-xs text-gray-400 mt-1">Diundi pada: {{ $winner->drawn_at->format('d M Y') }}</p>
                                </div>
                            </div>
                            @empty
